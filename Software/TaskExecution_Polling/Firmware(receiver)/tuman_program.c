...
void tuman_program(void)
{
    ...
    // Set fixed rx_window_upper, make sure the rx window must be valid
    rx_window_lower = get_cur_time();
    rx_window_upper = ts_add(rx_window_lower, 0x1F400 * 10);       // cover the whole schedule period
    set_ts_rx_window(rx_window_lower, rx_window_upper);
    

    while(1)
    {
        /*--> Receivce packet type check <--*/
        ret = receive_pkt_type_check();

        if (ret == 1)
        {
            /*--> Receivce TS packet <--*/
            u32* ts_rx_addr_tmp = (u32*)TS_SYSRAM_RX_PKT_ADDR;
            // disabled in Polling version
