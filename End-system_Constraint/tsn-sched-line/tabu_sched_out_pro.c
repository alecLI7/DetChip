...
u32 compute_flow_density_path_diff(struct sched_info sched, u16 offset, struct global_resource resource)
{
    ...
    for(i = 0; i < sched.path_len; i++)
    {
        ...
        {
            if(resource.cqf[node][port][j].used_len > 0)
            {
                if(resource.cqf[node][port][j].used_len > max_size)
                    max_size = resource.cqf[node][port][j].used_len;
                if(resource.cqf[node][port][j].used_len < min_size)
                    min_size = resource.cqf[node][port][j].used_len;
            }
        }
    }
...
u32 compute_flow_density_path_variance(struct sched_info sched, u16 offset, struct global_resource resource)
{
    ...
    for(i = 0; i < sched.path_len; i++)
    {
        ...
        for(j = 0; j < resource.cur_sched_slot_num; j++)
        {
            total_pkt_num += resource.cqf[node][port][j].used_len;
            used_slot_num++;
        }
    }
    ...
    for(i = 0; i < sched.path_len; i++)
    {
        ...
        for(j = 0; j < resource.cur_sched_slot_num; j++)
        {
            density += ((float)resource.cqf[node][port][j].used_len - average) * \
                        ((float)resource.cqf[node][port][j].used_len - average);
        }
    }
