...
int adjust_flow_with_offset_descend(struct sched_info *sched, struct global_resource *resource)
{
    ...
    int offset = 0;
    u16 latency = 0;
    int cur_offset = sched->offset;
    
    for(s = 1; s <= sched->period_slot; s++)
    {
        offset = (cur_offset - s) < 0 ? (sched->period_slot + cur_offset - s) : (cur_offset - s);
        latency = offset + sched->path_len + 1;
        if(latency <= sched->deadline_slot)
        {
            sched->flag = SUCCESS;
...
int adjust_flow_with_offset_random(struct sched_info *sched, struct global_resource *resource)
{
    ...
    temp_offset = sched->offset;
    p_flag.flag[temp_offset] = 1;
    p_flag.cur_slot_num++;
    offset = random(sched->period_slot);
/*
	while(1)
    {
        if(p_flag.total_slot_num == p_flag.cur_slot_num)
            break;
        offset = random(sched->period_slot);
...
//            printf("candidate: %d, out: %d\n", num, candidate_set[num].entry.out[i]);
            ...
            adjust_flow_with_offset_density(&candidate_set[num].sched_suc[start], &candidate_set[num].g_resource);
